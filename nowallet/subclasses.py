...
class SegwitBIP32Node(BIP32Node):
    def p2sh_p2wpkh_address(self) -> str:
        p2aw_script: bytes = self.p2sh_p2wpkh_script()
    ...
    def p2sh_p2wpkh_script(self) -> bytes:
        hash160_c: bytes = self.hash160(use_uncompressed=False)
        return ScriptPayToAddressWit(b'\0', hash160_c).script()

def main():
    svr: MyServerInfo = MyServerInfo("onion",
    ...
    tx_hex: str = "".join(hex_)
    tx: Tx = Tx.from_hex(tx_hex)

    utxos: List[LexSpendable] = [LexSpendable.promote(utxo)
        for utxo in tx.tx_outs_as_spendable()]
    utxos.sort()
    print(utxos)

    txouts: List[LexTxOut] = [LexTxOut(txout.coin_value, txout.script)
        for txout in tx.txs_out]
    txouts.sort()
    print([str(txout) for txout in txouts])

    secret: str = "CORRECT HORSE BATTERY STAPLE"
    mpk: SegwitBIP32Node = SegwitBIP32Node.from_master_secret(
        secret.encode("utf-8"))
